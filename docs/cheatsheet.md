...
Enter. По SSH включается безопасный Zsh-профиль с обычным completion: без P10K,
`fzf-tab`, autosuggestions и syntax highlighting. Он выглядит проще, но не
создаёт артефактов перерисовки на удалённых машинах.

## Prompt и Ghostty

Powerlevel10k локально настроен одной компактной строкой без рамок и правого
status-блока. Старые команды сжимаются благодаря transient prompt; ошибка
отмечается красным `❯`. По SSH включается простой safe-prompt с user, host и
каталогом — приоритетом там является стабильная перерисовка.
