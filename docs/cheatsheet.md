...
Enter. По SSH используется обычное completion Zsh: оно надёжнее для удалённых
терминалов.

## Prompt и Ghostty

Powerlevel10k настроен одной компактной строкой без рамок и правого status-блока.
Старые команды сжимаются благодаря transient prompt. Локально видны каталог и
Git, а hostname появляется только через SSH; ошибка отмечается красным `❯`.
