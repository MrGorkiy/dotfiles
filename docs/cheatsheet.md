...
Открыть из терминала: `th`. Редактировать: `the`. Команды ниже рассчитаны на
...
| `Tab` | fzf-tab: интерактивный выбор вариантов completion |
...
стрелкой `→` или End. `Tab` после `git`, `docker`, пути или опции включает
`fzf-tab`: печатайте часть названия, выбирайте стрелками, подтверждайте Enter.

## Prompt и Ghostty

Powerlevel10k настроен двухстрочно, без рамок и с transient prompt: старые
команды сжимаются, правая часть не разрастается. Это устойчивее при длинных
completion и SSH. Hostname появляется через элемент context на удалённых
соединениях.

Ghostty использует Catppuccin Mocha, Meslo Nerd Font Mono 15pt, небольшую
прозрачность и blur. После смены конфигурации перезапустите Ghostty или нажмите
`Cmd+Shift+,`.
...
3. Временно закомментируйте `fzf-tab` в массиве `plugins`, чтобы изолировать
   проблему.
4. Не используйте тяжёлую рамку/многострочный right prompt P10K.
...
Если terminfo отсутствует, для диагностики подключитесь так:

```bash
TERM=xterm-256color ssh host
```

Если это исправляет проблему, установите terminfo Ghostty на сервере; не
оставляйте подмену `TERM` как постоянное решение без необходимости.
